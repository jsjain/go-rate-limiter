...
func BenchmarkRedis_AllowN_HotKey(b *testing.B) {
	...
	l := NewLimiter(benchClient(b), WithRateLimit(PerSecond(1_000_000_000)))
	key := "bench:hot"
	...
	b.ResetTimer()
	for b.Loop() {
		if _, err := l.AllowN(ctx, key, 1); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkRedis_AllowN_ManyKeys_Parallel(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		i := 0
		for pb.Next() {
			if _, err := l.AllowN(ctx, benchKeys[i%len(benchKeys)], 1); err != nil {
				b.Fatal(err)
			}
			i++
		}
...
func BenchmarkRedis_AllowAtMost_HotKey(b *testing.B) {
	...
	b.ResetTimer()
	for b.Loop() {
		if _, err := l.AllowAtMost(ctx, key, limit, 1); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkRedis_AllowAtMostN_HotKey(b *testing.B) {
	...
	b.ResetTimer()
	for b.Loop() {
		if _, err := l.AllowAtMostN(ctx, key, 1); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkMem_AllowN_HotKey(b *testing.B) {
	ctx := context.Background()
	l := NewInMemoryLimiter(WithRateLimit(PerSecond(1_000_000_000)))
	defer l.Close()

	b.ReportAllocs()
	b.ResetTimer()
	for b.Loop() {
		if _, err := l.AllowN(ctx, "bench:hot", 1); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkMem_AllowN_HotKey_Parallel(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			if _, err := l.AllowN(ctx, "bench:hotpar", 1); err != nil {
				b.Fatal(err)
			}
		}
	})
...
func BenchmarkMem_AllowAtMostN_HotKey(b *testing.B) {
	...
	b.ResetTimer()
	for b.Loop() {
		if _, err := l.AllowAtMostN(ctx, "bench:atmostn", 1); err != nil {
			b.Fatal(err)
		}
	}
}
...
func BenchmarkMem_AllowN_Denied_Parallel(b *testing.B) {
	ctx := context.Background()
	l := NewInMemoryLimiter(WithRateLimit(PerSecond(1)))
	defer l.Close()
	l.AllowN(ctx, "bench:denied", 1) // exhaust

	b.ReportAllocs()
	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			if _, err := l.AllowN(ctx, "bench:denied", 1); err != nil {
				b.Fatal(err)
			}
		}
	})
